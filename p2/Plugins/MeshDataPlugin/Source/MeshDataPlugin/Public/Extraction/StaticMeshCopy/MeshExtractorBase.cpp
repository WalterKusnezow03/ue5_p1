...
void MeshExtractorBase::ExtractMeshDataPerSection(
...
void MeshExtractorBase::ExtractMeshDataPerSection(UStaticMeshComponent* comp, TArray<MeshData> &meshdata){
    if(comp){
...
void MeshExtractorBase::ExtractMeshDataPerSection(UStaticMesh *comp, TArray<MeshData> &meshdata){
    if(comp){
...
void MeshExtractorBase::ExtractSingleMeshData(
    TArray<UStaticMeshComponent*> &components, 
    MeshData &meshdata
){
    for (int i = 0; i < components.Num(); i++){
...
void MeshExtractorBase::ExtractSingleMeshData(UStaticMeshComponent* comp, MeshData &meshData){
    if(comp){
        if(UStaticMesh *inner = comp->GetStaticMesh()){
            int32 lod = 0;
...
void MeshExtractorBase::ExtractSingleMeshData(
    ...
    if(staticmesh){
        ...
        if(renderdata){
            ExtractSingleMeshData(renderdata, singleMeshData, LODIndex); //LOD 0 only.
        }
...
void MeshExtractorBase::ExtractSingleMeshData(
    ...
    if(renderdata){
        ...
        for (int32 i = 0; i < Indices.Num(); i += 3)
        {
            ...
            singleMeshData.append(v0, v1, v2);
            //singleMeshData.appendEfficent(v0, v1, v2);
        }   
    }
}

FVector MeshExtractorBase::ToVectorDouble(FVector3f vertex){
    return FVector(
        vertex.X,
        vertex.Y,
        vertex.Z

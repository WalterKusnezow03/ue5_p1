#include "WorldListWidgetActor.h"

#include "p2/ui/3Dui/GameStartRoom/WorldList/WorldListWidget.h"



void AWorldListWidgetActor::BeginPlay(){
    Super::BeginPlay();
    if (UWorldListWidget *widget = GetWidget<UWorldListWidget>())
    {
        widget->OnBeginPlay();
    }
}

void AWorldListWidgetActor::EndPlay(const EEndPlayReason::Type EndPlayReason){
    if (UWorldListWidget *widget = GetWidget<UWorldListWidget>())
    {
        widget->OnEndPlay();
    }
    Super::EndPlay(EndPlayReason);
}



//init on beginplay
void AWorldListWidgetActor::CreateWidgetMeshData(){
    if(!bMeshDataCreated && Widget){
        float xMax = 300.0f;
        float yMax = 600.0f;

        /*
        1-->2
        |   |
        0<--3
        */

        FVector v0(0, 0, 0);
        FVector v1(0, 0, yMax);
        FVector v2(0, xMax, yMax);
        FVector v3(0, xMax, 0);

        MeshData &data = Widget->GetMeshDataRef();
        data.append(v0,v1,v2);
        data.append(v0,v2,v3);
        data.centerMesh();

        MMatrix scale;
        float factor = 0.1f;
        scale.scale(factor, factor, factor);
        data.transformAllVertecies(scale);

        FVector2D uv0(0, 1);
        FVector2D uv1(0, 0);
        FVector2D uv2(1, 0);
        FVector2D uv3(1, 1);
        data.appendUvs(uv0, uv1, uv2);
        data.appendUvs(uv0, uv2, uv3);

        data.calculateNormals();
        //data.flipNormals(); //shit happens
        data.flipWindingOrder();

        bMeshDataCreated = true;

        SetDrawSize(FVector2D(xMax, yMax));

        //400cm
        ScaleMeshDataToMaxCm(400.0f);
    }
}
